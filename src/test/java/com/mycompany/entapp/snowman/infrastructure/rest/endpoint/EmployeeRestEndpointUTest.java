...
public class EmployeeRestEndpointUTest {
    ...
    public void testCreateEmployeeShouldCreateEmployee() {
        ...
        try (MockedStatic<EmployeeResourceMapper> mockedStatic = Mockito.mockStatic(EmployeeResourceMapper.class)) {
            mockedStatic.when(() -> EmployeeResourceMapper.mapEmployeeResourceToEmployee(employeeResource)).thenReturn(employee);
            Mockito.doNothing().when(employeeService).createEmployee(employee);

            systemUnderTest.createEmployee(employeeResource);

            mockedStatic.verify(() -> EmployeeResourceMapper.mapEmployeeResourceToEmployee(employeeResource));
